...
const overviewLists = ['clayton', 'personal', 'work'];
...
function taskInWorkspace(task, workspace = activeWorkspace) { const logicalList = task.done ? (task.previousList || task.list) : task.list; const normalizedList = String(logicalList).toLowerCase(); return workspace === 'overview' ? overviewLists.includes(normalizedList) : workspace === 'work' ? normalizedList === 'work' : personalWorkspaceLists.some((list) => list.toLowerCase() === normalizedList); }
